var soft = {
    ...
    get_list: function (page, type, search) {
        ...
        bt.soft.get_soft_list(page, type, search, function (rdata) {
        	//console.log(rdata);
            if (rdata.pro >= 0) {
                $("#updata_pro_info").html('<div class="alert alert-success" style="margin-bottom:15px"><strong>当前为专业版，专业版可以免费使用专业版插件，过期时间：'+ (rdata.pro > 0 ?bt.format_data(rdata.pro, 'yyyy/MM/dd'):'永久授权') + '<font size="" color="#ff0000">&nbsp;&nbsp;&nbsp;若防火墙无法使用，请先面板安装防火墙，然后ssh执行“bt”命令选择33修复防火墙！</font>' +'</strong><button style="margin-left:8px;display:'+ (rdata.pro>0?'inline-block':'none') +';" class="btn btn-success btn-xs va0 updata_pro" onclick="bt.soft.updata_pro()" title="续费专业版">立即续费</button>');
            }
            if (rdata.ltd >= 0) {
